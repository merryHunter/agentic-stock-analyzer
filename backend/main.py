...
class PricePrediction(BaseModel):
    ...
    overall_change: int = Field(description="The estimated stock price change value (positive or negative) for the next day.")
...
async def search_news(filter: NewsFilter):
    ...
    try:
        news = get_news_for_ticker_newsapi(filter.ticker, filter.company_name)
...
async def analyze_news_with_llm(news: List[dict], ticker: str, company_name: str):
    ...
    logger.info(f"Using GPT model: {gpt_model_name}")
...
def get_news_for_ticker_newsapi(ticker, company_name=None):
    ...
    return [
        {
            ...
            "movement": np.random.randint(-10, 10)
        }
